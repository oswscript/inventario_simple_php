...
if(isset($_POST['act'])) {
	...
	if($_POST['act'] == '2') {
		...
		$role = $role->role;
		if($_users->delete_user($_POST['id']) == true) die('1');
		...
		/*if($role == 1 || $role == 2 || $role == 3) {
			...
			if($_session->get_user_role() == 1 || $_session->get_user_role() == 2 || $_session->get_user_role() == 3) {
				if($_users->delete_user($_POST['id']) == true)
					die('1');
			}
			die('wrong');
...
<body>
	<div id="main-wrapper">
		...
		<div class="wrapper-pad">
			...
			if($total_items == 0)
				echo 'No hay datos<br /><br /><br />';
			else{
			?>
			<table border="1" rules="rows" id="users">
				<thead>
					<tr>
						<?php /* <td width="5%">ID</td> */ ?>
						<td width="18%">Nombre</td>
						<td width="7%">Ced.</td>
						<td width="8%">Usu.</td>
						<td width="22%">Correo</td>
						<td width="10%">Rol</td>
						<td width="14%">Reg.</td>
...
?>
					<tr data-type="element" data-id="<?php echo $user->id; ?>">
						<?php /*<td class="hover" data-type="id"  style="font-size: 9pt"><?php echo $user->id; ?></td> */?>
						<td class="hover" data-type="name" style="font-size: 9pt"><?php echo $user->name; ?></td>
						<td class="hover" data-type="name" style="font-size: 9pt"><?php echo $user->dni; ?></td>
						<td class="hover" data-type="username" style="font-size: 9pt"><?php echo $user->username; ?></td>
						<td class="hover" data-type="email" style="font-size: 9pt"><?php echo $user->email; ?></td>
						<td class="hover" data-type="role" style="font-size: 9pt"><?php echo $_users->parse_role($user->role); ?></td>
