...
if(isset($_POST['act'])) {
	if($_POST['act'] == '1') {
		...
		if($_cats->cat_exists_edit($name,$catid) == true)
			die('2');
			...
			<div class="center">
				<div class="form">
					<form method="post" action="" name="edit-cat" data-id="<?php echo $cat->id; ?>">
						...
						<!--Category Place:<br />
						<div class="ni-cont">-->
							<input type="hidden" name="ncat-place" class="ni" value="<?php echo $cat->place; ?>" />
						<!--</div>-->
						<span class="ncat-desc-left">Descripción (<?php echo 400-strlen($cat->descrp); ?> caracteres restantes):</span><br />
						<div class="ni-cont">
							<textarea name="ncat-descrp" class="ni"><?php echo $cat->descrp; ?></textarea>
						</div>
						<input type="submit" name="ncat-submit" class="ni btn blue" value="Guardar" />
					</form>
				</div>
			</div>
		</div>
