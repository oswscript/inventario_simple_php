$('document').ready(function() {
	...
	$('a[name=delete-all]').click(function(evt) {
		...
		if(confirm('Esta seguro que desea borrar '+selected.length+' productos?')) {
			...
			$.post('items.php', {
				...
				if(data == 'wrong') {
					die('Algo va mal. Por favor vuelva a intentar');
					return false;
				}
	...
	function submit_check(from, type, t) {
		...
		if(type == 'Agregar') var act = 3;
		else if(type == 'Restar') var act = 4;
		
		if(val == '0' || val == '') {
			alert('Por favor agrege un numero de productos para '+type);
			return false;
		}
		
		if(type == 'out') {
			if(parseInt(prev_value) < val){
				alert('Usted no puede sacar más de '+prev_value+' productos');
				return false;
			}
		}
		...
		$.post('items.php', {
			...
			if(data == '2') {
				alert('No puede modificar más de'+prev_value+' productos');
				return false;
			}else if(data == '1'){
	...
	$('form[name=searchf]').submit(function(evt) {
		...
		$.post('items.php', {
			...
			if(data == '2') {
				alert('Ningún producto coincide con tu búsqueda');
				$('.loader').fadeOut(200);
				return false;
			}
