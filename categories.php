...
if(isset($_POST['act'])) {
	...
	if($_POST['act'] == '2') {
		...
		if($_cats->delete_cat_item($_POST['id']) == true)
			die('1');
		die('wrong');
	}
}
...
<body>
	<div id="main-wrapper">
		...
		<div class="wrapper-pad">
			...
			<?php
			if($c_cats == 0)
				echo '<br /><br />No hay categorías registradas';
			else{
			?>
			...
			<table border="1" rules="rows" id="categories">
				<thead>
					<tr>
						...
						-->
						<td width="14%">Produc. asignados</td>
						<td width="17%">Total U.</td>
						<?php if($role == 1 || $role == 2 || $role == 3){ ?>
							<td width="14%">Acciones</td>
						<?php } ?>
					</tr>
				</thead>
				<tbody>
...
?>
					<tr data-type="element" data-id="<?php echo $cat->id;?>">
						...
						<?php /* <td class="hover" data-type="id"><?php echo $cat->id; ?></td> */?>
						...
						<td><?php echo $_items->get_cat_tot_items($cat->id); ?></td>
							<?php
							if($role == 1 || $role == 2 || $role == 3)
								echo '<td><a style="background-color: #8CCBFF; padding:4%; font-weight:bold;" href="edit-category.php?id='.$cat->id.'" name="c3" title="Editar categoría"><i class="fa fa-pencil"></i>Editar</a>';
							//if($role == 1 || $role == 2 || $role == 3)
								//echo '<a href="logs.php?catid='.$cat->id.'" name="c4" title="Log of this category"><i class="fa fa-file-text-o"></i></a>';
							if($role == 1 || $role == 2 || $role == 3)
								echo '<a style="background-color: #FF8989; padding:4%; font-weight:bold;" href="" name="c5" title="Borrar categoría"><i class="fa fa-close"></i>Borrar</a></td>';
							?>
