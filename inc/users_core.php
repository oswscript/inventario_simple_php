...
class Users {
	...
	//Listado de usuarios y paginas por paginacion
	...
	/*public function search($string, $page, $items_per_page) {
		...
		$role = $this->session->get_user_role();
		if($role == 1)
			$prepared = $this->prepare("SELECT * FROM invento_users WHERE username LIKE ? OR dni LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ? ORDER BY id DESC LIMIT ?,?", 'search()');
		elseif($role == 2)
			$prepared = $this->prepare("SELECT * FROM invento_users WHERE (role=3 OR role=4) AND (username LIKE ? OR dni LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?) ORDER BY id DESC LIMIT ?,?", 'search()');
		elseif($role == 3)
			$prepared = $this->prepare("SELECT * FROM invento_users WHERE (role=4) AND (username LIKE ? OR dni LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?) ORDER BY id DESC LIMIT ?,?", 'search()');
		else
			return false;
	...
	//cantidad de usuarios de la busqueda
	public function count_users_search($string) {
		$s = "%$string%";
		
		$role = $this->session->get_user_role();
		if($role == 1)
			$prepared = $this->prepare("SELECT COUNT(*) as c FROM invento_users WHERE username LIKE ? OR dni LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?", 'count_users_search()');
		elseif($role == 2)
			$prepared = $this->prepare("SELECT COUNT(*) as c FROM invento_users WHERE (role=3 OR role=4) AND (username LIKE ? OR dni LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?)", 'count_users_search()');
		elseif($role == 3)
			$prepared = $this->prepare("SELECT COUNT(*) as c FROM invento_users WHERE role=4 AND (username LIKE ? OR dni LIKE ? OR name LIKE ? OR email LIKE ? OR date_added LIKE ?)", 'count_users_search()');
		else
			return 0;
		
		$this->bind_param($prepared->bind_param('sssss', $s, $s, $s, $s, $s), 'count_users_search()');
	...
	//confirmar si el usuario existe por nombre de usuario
	...
	//confirmar si el usuario existe por nombre de cedula
	public function dni_exists($dni) {
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM invento_users WHERE dni=?", 'dni_exists()');
		$this->bind_param($prepared->bind_param('s', $dni), 'dni_exists()');
		$this->execute($prepared, 'dni_exists()');
		...
		//confirmar si el usuario existe por nombre de ID
	...
	public function get_user($userid) {
		...
		return $row;$q = $this->mysqli->query($query);
	}
	//editar usuario
	...
	//Rol de usuarios segun el numero
	public function parse_role($role) {
		if($role == 1)
			return 'Administrador';
		elseif($role == 2)
			return 'Supervidor General';
		elseif($role == 3)
			return 'Supervisor';
		elseif($role == 4)
			return 'Empleado';
