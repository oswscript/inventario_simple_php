...
class Items {
	...
	//Capturar items encontrados en la busqueda
	public function count_items_search($string) {
		$s = "%$string%";
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM invento_items WHERE id LIKE ? OR name LIKE ? OR code LIKE ? OR descrp LIKE ? OR date_added LIKE ? OR category IN (SELECT id FROM invento_categories WHERE name LIKE ?)", 'count_items_search()');
		$this->bind_param($prepared->bind_param('ssssss', $s, $s, $s, $s, $s, $s), 'count_items_search()');
...
//buscar si el produto existe
	public function item_exists($code) {
		$prepared = $this->prepare("SELECT COUNT(*) as c FROM invento_items WHERE code=?", 'item_exists()');
		$this->bind_param($prepared->bind_param('s', $code), 'item_exists()');
		$this->execute($prepared, 'item_exists()');
	...
	//Capturar nombre de la categoria por el id
	...
	//Capturar el nombre del producto por ID
	...
	//Cantidad de items en la categoria
	...
	//nuevo producto
	public function new_item($name, $code, $desc, $cat, $qty, $price) {
		...
		$prepared = $this->prepare("INSERT INTO invento_items(name,code,descrp,category,qty,price,date_added) VALUES(?,?,?,?,?,?,?)", 'new_item()');
		$this->bind_param($prepared->bind_param('sssiiis', $name, $code, $desc, $cat, $qty, $price, $date), 'new_item()');
		$this->execute($prepared, 'new_item()');
		
		return true;
	}
	//capturar datos del producto por id
